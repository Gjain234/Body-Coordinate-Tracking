import json
body_parts = ['nose', 'neck', 'right_shoulder', 'right_elbow', 'right_wrist', 'left_shoulder', 'left_elbow',
              'left_wrist', 'mid_hip', 'right_hip', 'right_knee', 'right_ankle', 'left_hip', 'left_knee', 'left_ankle',
              'right_eye', 'left_eye', 'right_ear', 'left_ear', 'l_big_toe', 'left_small_toe', 'left_heel',
        ...
        if wrt == 'absolute':
            for key in body1.coord:
                if body1.coord[key] is None or body2.coord[key] is None:
                    ...
                    new_dict[key] = (body1.coord[key][0] - body2.coord[key][0], body1.coord[key][1] - body2.coord[key][1])
            return new_dict
        if wrt == 'normalized':
            for key in body1.coord_normalized:
                if body1.coord_normalized[key] is None or body2.coord_normalized[key] is None:
                    ...
                    new_dict[key] = (body1.coord_normalized[key][0] - body2.coord_normalized[key][0], body1.coord_normalized[key][1] - body2.coord_normalized[key][1])
            return new_dict
...
def get_scaled_dict_wrt_corner(body, scale_x, scale_y, origin_x, origin_y):
    ...
    try:
        for part in body.coord_normalized:
            if (body.coord_normalized[part] != None):
...
def get_total_error(body1, body2):
    error_dict = subtract_dict_coord(body1, body2)
    total_error = 0
...
def get_part_comments(error_dict, part, error):
    try:
        if error_dict[part]:
            final_message = ''
...
def get_full_body_comments(body1, body2, ball_error, measurement):
    ...
    for key in error_dict:
        if error_dict[key]:
            if get_part_error(error_dict, key) > ball_error:
                all_comments += get_part_comments(error_dict, key, ball_xy_component) + ' '
    return all_comments
...
class Body:
    ...
    def __init__(self, file_name, wrt_part='neck', normalize_type='y'):
        try:
            ...
            print('This is not a valid file for finding body part coordinates.')
        except TypeError:
